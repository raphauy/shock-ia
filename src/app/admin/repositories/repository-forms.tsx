...
export function RepositoryForm({ closeDialog }: Props) {
  ...
  const onSubmit = async (data: RepositoryFormValues) => {
    ...
    try {
      ...
      toast({ title: "FC creada correctamente" })
      closeDialog()
      router.push(`/admin/repositories/${created.id}`)
    } catch (error: any) {
      if (error.message.includes('Ya existe una FC con el nombre') || 
          error.message.includes('Ya existe una función con')) {
        toast({ 
        ...
        toast({ 
          title: "Error", 
          description: error.message || "Ocurrió un error al crear la FC", 
          variant: "destructive" 
        })
      }
    } finally {
...
export function DeleteRepositoryForm({ id, closeDialog, redirect, clientCount = 0 }: DeleteProps) {
  const [loading, setLoading] = useState(false)
  const router= useRouter()

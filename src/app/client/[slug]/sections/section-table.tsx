...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="secuence filter..."
              value={(table.getColumn("secuence")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("secuence")?.setFilterValue(event.target.value)}                
          />
          
      
          <Input className="max-w-xs" placeholder="tokenCount filter..."
              value={(table.getColumn("tokenCount")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("tokenCount")?.setFilterValue(event.target.value)}                
          />
